...
public class ApacheConnectBean2  {
   ...
   // private Logger logger = null;
    ...
    public ApacheConnectBean2() {
        ...
        config = RequestConfig.custom().setRedirectsEnabled(true)
                ...
                .build();
      //  logger = Logger.getLogger(ApacheConnectBean2.class);
    }
    ...
    public byte[] doConnectPut(String path, byte[] entityBody, String contentType)
            throws HttpConnectException {
          byte[] result = null;
           HttpPut put = null;
           this.initProperties();
           ...
           this.setRequestHeaders(put, contentType);   
           CloseableHttpResponse resp = this.doExecute(put,"doConnectPut");
           result = this.getBinaryResult(resp);   
           try {
             resp.close();
           }
   ...
   private void setRequestHeaders(HttpRequestBase request, String contentType){
        ...
        if(this.accept.isEmpty())
        	request.setHeader("Accept",ApacheConnectBean2.AcceptDefault);
        else
       ...
       if(this.client == null)
        ...
        try {
            ...
            throw new HttpConnectException(timeEx,
                    "SocketTimeout: " + timeEx.getMessage(),
                    display,
                    this.module + "#" + method);
        }
            ...
            client = null;
            throw new HttpConnectException(timeEx,
                    "ConnectTimeout: " + timeEx.getMessage(),
                    display,
                    this.module + "#" + method);
        }
            ...
            throw new HttpConnectException(protEx,
                    "ClientProtocolException: " + protEx.getMessage(),
                    display, 
                    this.module + "#" + method);
        }
        catch(IOException ioEx){
            ...
            throw new HttpConnectException(ex,
                   "Unknown Exception: " +  ex.getMessage(),
                    display, 
                    this.module + "#" + method);
        }
        return resp;
  ...
  /*  private void parseElements(List<String>values, Header h){
    	HeaderElement[] elements = h.getElements();
    	if(elements == null)
    		logger.info("parseElements: elements is null");
    	else logger.info("parseElements: elements size is " + elements.length);
    	for(int i=0; i < elements.length; i++){
    		values.add(elements[i].getValue());
    		logger.info("parseElements:" + elements[i].getValue());
    	}
    }*/
